...
export async function postMessage(req: Request, res: Response) {
  try {
    ...
    if (!emailValidator.validate(email))
      return res.status(400).json({ error: true, message: "Please, provide the correct email!" });
    if (text.length > 1000)
      return res
        .status(400)
        .json({ error: true, message: "Your message is too long! The message must be less than 1000 symbols!" });
